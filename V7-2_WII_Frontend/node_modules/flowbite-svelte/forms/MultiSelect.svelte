...
// Container
const multiSelectClass = 'px-3 py-1 min-h-[2.5rem] relative border border-gray-300 flex items-center rounded-lg gap-2 dark:border-gray-600 focus-within:ring-1 focus-within:border-primary-500 ring-primary-500 dark:focus-within:border-primary-500 dark:ring-primary-500';
// Dropdown
...
// Items
const itemsClass = 'py-2 px-3 rounded-lg text-gray-600 hover:text-blue-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:text-gray-300 dark:hover:bg-gray-600';
// Selected items
const itemsSelectClass = 'py-2 px-3 rounded-lg bg-gray-100 cursor-pointer text-black hover:text-black dark:text-white dark:bg-gray-600 dark:hover:text-white';

const disabledItemClass="py-2 px-3 rounded-lg text-gray-600 cursor-not-allowed opacity-50";
onMount(() => {
    if (value.length) {
        items.map((item) => {
...
const disabledMultiSelectDropdown=twMerge('absolute z-50 p-3 flex flex-col gap-1 max-h-64 bg-white border border-gray-300 dark:bg-gray-700 dark:border-gray-600 left-0 top-[calc(100%+1rem)] rounded-lg overflow-y-scroll w-full', dropdownClass);
...
<select bind:value hidden multiple on:change on:input>
  {#each items as { value, name }}
    <option {value}>{name}</option>
  {/each}
</select>
<!-- svelte-ignore a11y-click-events-have-key-events -->
<div on:click={() => (show = !show)} on:focusout={() => (show = false)} tabindex="-1" role="listbox" class={twMerge(multiSelectClass, sizes[size], $$props.class,$$restProps)}>
...
{#if show}
    ...
    <div on:click|stopPropagation role="presentation" class={multiSelectDropdown}>
      {#each items as item (item.name)}
        ...
        {#if selectItems.includes(item)}
            <div on:click={() => selectOption(item)} role="presentation" class={itemsSelectClass}>
            {item.name}
            </div>
        {:else}
            <div on:click={() => selectOption(item)} role="presentation" class={itemsClass}>
            {item.name}
            </div>
        {/if}
      {/each}
    ...
    {:else}
        <div on:click|stopPropagation role="presentation" class={disabledMultiSelectDropdown} >
      {#each items as item (item.name)}
        <!-- svelte-ignore a11y-click-events-have-key-events -->
        {#if selectItems.includes(item)}
            <div on:click={() => selectOption(item)} role="presentation" class={itemsSelectClass}>
        ...
        {:else}
            <div role="presentation" class={disabledItemClass}>
            {item.name}
            </div>
        {/if}
      {/each}
